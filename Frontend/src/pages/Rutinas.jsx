...
const Rutinas = () => {
  ...
  const categorias = [
    ...
    { nombre: "PRÓXIMAMENTE", imagen: routine2, disabled: true },
    { nombre: "PRÓXIMAMENTE", imagen: routine3, disabled: true },
    { nombre: "PRÓXIMAMENTE", imagen: routine5, disabled: true },
        ...
        {categorias.map((categoria, index) => {
          ...
          return (
            <CategoryWrapper
              {...linkProps}
              key={index}
              className={`relative block overflow-hidden rounded-lg ${categoria.disabled ? "cursor-default opacity-70" : "cursor-pointer"}`}
            >
              <img
                src={categoria.imagen}
                alt={categoria.nombre}
                className={`object-cover w-full max-h-40 img-gradient ${categoria.disabled ? "opacity-70" : "opacity-40"}`}
              />
